...
from services.dtc_database import CATEGORY_NAMES, CATEGORY_ICONS
...
class DTCPage(QWidget):
    ...
    def read_dtcs(self):
        ...
        try:
            confirmed_codes = self.protocol.trouble_codes(pending=False)
            pending_codes = self.protocol.trouble_codes(pending=True)
            permanent_codes = self.protocol.trouble_codes(permanent=True)
        ...
        from services.dtc_database import build_dtc

        confirmed = set(confirmed_codes)
        permanent = set(permanent_codes)

        all_codes = list(dict.fromkeys(confirmed_codes + pending_codes + permanent_codes))
    ...
    def apply_filter(self, category_label):
        ...
        for label, code_letter in CATEGORY_LOOKUP.items():
            if code_letter:
                counts[label] = len([d for d in self.dtcs if d.category_letter == code_letter])
    ...
    def show_freeze_frame(self, row, _column):
        ...
        # Fotografia dos sensores mais relevantes no momento da falha.
